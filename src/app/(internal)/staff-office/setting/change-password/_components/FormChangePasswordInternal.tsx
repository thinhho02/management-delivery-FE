...
const FormChangePasswordInternal = () => {
...
const dialog = createOverlay<DialogProps>((props) => {
    ...
    const handleSendOTP = useCallback(() => {
        startTransition(async () => {
            ...
            const verifyEmail = await create("/employee/verify-email", { email })
    ...
    const submitForm2 = form2.handleSubmit(async (dataForm2) => {
        ...
        const res = await update<{ message: string }>("/employee/update-password", { password: dataForm2.newPassword })
        if (!res.success) {
            toaster.create({
                id: `Change-Pass-E-${Date.now}`,
                type: 'error',
                                                ...
                                                <Controller
                                                    ...
                                                    render={({ field }) => (
                                                        <Group attached w={'full'}>
                                                            <Input type="text" ref={inputEmailRef} readOnly={renderVerify} value={field.value} flex={1} outline={'none'} placeholder='Nhập địa chỉ Email của bạn' onChange={(e) => field.onChange(e.target.value)} />
...
export default FormChangePasswordInternal