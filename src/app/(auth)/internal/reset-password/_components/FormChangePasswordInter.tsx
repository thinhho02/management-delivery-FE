...
const FormChangePasswordInter = ({ token }: { token: string }) => {
    ...
    const submitForm = handleSubmit(async (dataForm) => {
        ...
        const res = await update<{ message: string }>("/employee/reset-password", { newPassword: dataForm.newPassword, token: token })
        if (!res.success) {
            toaster.create({
                id: `Change-Pass-E-${Date.now}`,
                type: 'error',
...
export default FormChangePasswordInter