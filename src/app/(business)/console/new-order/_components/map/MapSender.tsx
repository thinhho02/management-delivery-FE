...
const MapSender = memo(({ lngLat, setLngLat, setValue, wardId, setAddressMarker }: Props) => {
    ...
    const fetchReverseGeo = async (latlng: string) => {
        ...
        try {
            ...
            features.map((feature: any) => {
                ...
                setValue("senderAddress", value,{ shouldDirty: true, shouldValidate: true })
            })
    ...
    useEffect(() => {
        ...
        if (!markerRef.current) {
            ...
            marker.on("dragend", async () => {
                ...
                setValue("senderLngLat", valueLngLat)
        ...
        if (!checkPoint) {
            ...
            setValue("senderLngLat", '')
        } else {
            setValue("senderLngLat", lngLat)
        }
    ...
    useEffect(() => {
        ...
        const handleClick = async (e: mapboxgl.MapMouseEvent & mapboxgl.MapEvent) => {
            ...
            // Cập nhật value form
            setLngLat(newValue);
            setValue("senderLngLat", newValue);
    ...
    useEffect(() => {
        ...
        const safeAddLayer = () => {
            ...
            if (map.getSource("MapUserReceiver")) {
                (map.getSource("MapUserReceiver") as mapboxgl.GeoJSONSource).setData(dataGeojson);
                return; // Không addSource, addLayer nữa
            }
            map.addSource("MapUserReceiver", {
                type: "geojson",
                data: dataGeojson,
            });
            map.addLayer({
                id: `MapUserReceiver-fill`,
                type: "fill",
                source: `MapUserReceiver`,
                paint: {
                    "fill-color": '#00ffff',
                    "fill-opacity": 0.2,
                },
